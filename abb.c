...
{ //Se devuelve el elemento más a la izquierda
    abb aux;
    tipoelem ele;
    if (es_vacio((*A)->izq))
    { //Si izquierda vacía, se devuelve valor nodo actual A
        ele = (*A)->info;
        aux = *A;
        *A = (*A)->der;
        free(aux);
...
void suprimir(abb *A, tipoelem E)
{
    ...
    {                                //ni derecha ni izquierda esta vacio, busco mínimo subárbol derecho
